 ...
 *               		   InputStream (바이트 스트림: 1byte씩 이동)/ Reader (문자 스트림)
 *               												--------------------
 *               			메모리에 있는 데이터
 *               			파일에 있는 데이터
 *               			소켓에 있는 데이터 (네트워크)
 ...
 *  BufferdInputStream  BufferedReader        
 *  -----------------------------------
 ...
 *         ***mkdir() : 디렉토리 만들기
 *       = 파일 삭제
 *         ***delete() => 한개만 (파일이 있는 디렉토리는 삭제 안된다) 
 *         					  => 파일 먼저 삭제 후 디렉토리 삭제
 *      = 파일이나 디렉토리의 존재 여부
 *        ***boolean exists()
 *      = 수정날짜
