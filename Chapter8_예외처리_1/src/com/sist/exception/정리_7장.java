 ...
 *   객체지향의 3대요소
 *   1. 데이터 보호 목적
 *      캡슐화 : 변수를 은닉화 => 매소드를 통해서 접근이 가능하게 만든다
 *             ---------                  ---------
 *              private                     public
 *           => 변수는 2가지 기능만 수행
 *              메모리 저장 / 메모리에서 데이터 읽기
 ...
 *      2) public : 공개(패키지 상관없이 모든 클래스에 접근이 가능)
 ...
 *      class A
 *      {
 *        B b=new B(); //포함 //특별한 경우가 아니면 재정의하지 않는다
 *      }
 *   3. 수정,추가 목적 (유지보수)
 ...
 *    클래스의 종류
 *    = 추상클래스
 *    = 인터페이스 => 추상클래스의 단점을 보완 (클래스와 동일하기 취급)
 *      ** 클래스(단일) / 인터페이스(다중)
 *   ----- 설계도(구현이 안된 상태) => 미완성 클래스 (자신이 메모리 할당을 할 수 없다)
 *   ----- 여러개의 관련된 클래스를 모아서 한 개의 객체 명으로 제어
 *   -------------------------------------------------------------------
 *                       추상클래스              인터페이스
 *   -------------------------------------------------------------------
 *    상속관계           단일 상속                다중 상속
 ...
 *   
 *        추상클래스
 *  [접근지정어] abstract class CalssName
 *  {
 *    ---------------------------------
 *    변수 
 *       인스턴스 변수
 ...
 *     --------------------------------
 *   }
 *   ==> 인터페이스 변수나 메소드가 public (연결목적 => 언제든 연결이 가능)
 *   -------------------------------------------------------------------
 *   
 *     상속       
 *               extends
