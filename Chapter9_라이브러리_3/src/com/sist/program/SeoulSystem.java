...
//VO => DAO => 전송 => 관련된 데이터는 무조건 묶어서 보낸다 (VO)=class
import java.util.*;
import java.io.*;
public class SeoulSystem {
	...
	{
		return (int)(Math.ceil(datas.length/10.0)); //ceil = 올림함수
		             //--------------------------------- 27.3=> 28
	...
	public void process()
	{
		...
		while(true)
		{
			...
			else if(menu==4)
			{
				...
				for(SeoulLocationVO vo:data)
				{
					System.out.println(vo.getNo()+"."+vo.getName()+" 주소="+vo.getAddress());
				}
			...
			else if(menu==1)
			{
				...
				{
					System.out.println("없는 페이지입니다"); //오류조건
					continue;
				}
				break;
				}
				...
				for(SeoulLocationVO vo:data)
				{
					if(vo!=null)
					{
					System.out.println(vo.getNo()+"."+vo.getName());  //nullpoint 오류처리 //배열일 떄 사용
					}
