 ...
 *   	-----------------------------------------------------
 * 		지역변수(25page)  ==> 메소드 호출될 때 메소드안에서만 사용이 가능
 * 		변수 : 한개의 데이터를 저장하는 공간
 * 		우선 => 변수명이 동일할 때 우선 순위
 * 		=> 지역변수, 매개변수
 * 		--------- 반드시 초기화후에 사용
 ...
 * 			3) 특수문자 사용 (_ , $)
 * 			4) 키워드는 사용 불가 (메소드명, 클래스명 키워드가 아니다)
 * 			5) 변수에 공백이 없다
 * 			-------------------------------------------------- 오라클 , 자바스크립트에서도 동일
 ...
 *        	오라클 : = , <> , ^=
 *        	자바스크립트 : === , !==
 *        => 논리연산자 : &&(직렬) => 두개가 true일 때 true , || (병렬) => 둘중에 1개이상 true 
 *        												*** 효율적인 연산
